"""
Logging configuration for CourtListener MCP with file-based audit trail.
...
def setup_logging(log_level: str = "INFO"):
    """
    Configure logging for CourtListener MCP with file-based audit trail.

    Creates two log files in ~/.courtlistener_mcp/logs/:
    - courtlistener_mcp.log: General application logs (10MB max, 5 backups)
    ...
    """
    logs_dir = Path.home() / ".courtlistener_citations_mcp" / "logs"
    ...
    app_log_file = logs_dir / "courtlistener_citations_mcp.log"
    ...
    if hasattr(os, 'chmod'):
        for log_file in [app_log_file, security_log_file]:
            try:
                ...
                print(
                    f"Warning: Could not set file permissions on {log_file}: {e}",
                    file=sys.stderr,
                )
